...
private bookingList:Booking[]=
  ...
  deleteBooking(id:number) {
    const index = this.bookingList.findIndex(book => book.id === id);
    if (index !== -1) {
  ...
  addBooking(booking:Booking){
    let index:number = this.bookingList.findIndex(bookList => booking.id == bookList.id);

    if(index<=-1){
      booking.id = Math.trunc(Math.random()*100)+1;
      this.bookingList.push(booking);
    }else{
      this.bookingList[index]= booking;
    }
  }
